...
const createMap = async (req, user) => {
  // used within saveUserMap
  const {
    title,
    isPublic,
    mapType,
  ...
  const map_data = await MapData.create({
    // create the map data and store it in the database
  ...
  const map = await Map.create({
    // create the map and add the reference to the corresponding map data
    user_id: user._id,
    title: title,
    isPublic: isPublic,
    mapType: mapType,
...
const getMap = asyncHandler(async (req, res) => {
  ...
  if (!mapWithDetails) {
    return res
      .status(404)
      .json({ message: "Could not find map or related data" });
  }
  ...
  if (isNeutral) {
    ...
    if (amount > 0)
      map = await Map.findByIdAndUpdate(
        map_id,
        { $inc: { likes: amount }, $pull: { userDislikes: user_id } },
        { new: true }
      );
    else
      map = await Map.findByIdAndUpdate(
        map_id,
        { $inc: { likes: amount }, $pull: { userLikes: user_id } },
        { new: true }
      );
  } else {
    if (amount > 0)
      map = await Map.findByIdAndUpdate(
        ...
        {
          $inc: { likes: amount },
          $push: { userLikes: user_id },
          $pull: { userDislikes: user_id },
        },
        { new: true }
      );
    else
